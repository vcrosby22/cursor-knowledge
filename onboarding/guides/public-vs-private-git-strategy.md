...
| **Onboarding + product-management** (triggers, GLOSSARY, PM KB, playbooks) | **`cursor-knowledge-public/`** (git submodule at monorepo root) | **`cursor-knowledge`** (e.g. `git@github.com:vcrosby22/cursor-knowledge.git`) | **Public** |
...
- Edit **`onboarding/`** / **`product-management/`** only via the **`cursor-knowledge`** public repo (submodule **`cursor-knowledge-public/`**), or run **`scripts/bundle_cursor_knowledge_public_repo.py`** into a clone and push — do not expect those trees to live only in the private remote.
...
2. **Public:** small repo (e.g. `cursor-knowledge` template pattern) with only approved paths — copy or subtree export; **never** copy `BACKLOG*.md`, `BACKLOG_INBOX.md`, `journal/`, `profile/` unless intentionally sanitized.
...
- **2026-03-20** — **`onboarding/`** + **`product-management/`** canonical in public **`cursor-knowledge`**; private monorepo uses submodule + symlinks + **`bundle_cursor_knowledge_public_repo.py`**.
